...
function getScriptTemplate(name, nativeName, code, charsJson) {
  return `/**
 * Special Characters Helper - ${name} (${code})
 * 
 ...
 *   <script src="special-chars-${code}.js"></script>
 */
(function () {
  'use strict';
  ...
  // Character mapping for ${name} (${nativeName})
  const CHAR_MAP = ${charsJson};
...
const outDirs = [
  path.join(__dirname, '..', 'public', 'special-chars'),
  path.join(__dirname, '..', 'special-chars')
];
...
for (const [code, config] of Object.entries(CHAR_MAPS)) {
  const content = getScriptTemplate(config.name, config.nativeName, config.code, JSON.stringify(config.chars, null, 4));
  outDirs.forEach(dir => {
    const filePath = path.join(dir, `special-chars-${code}.js`);
